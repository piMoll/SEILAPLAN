...
class SeilaplanPluginDialog(QDialog, FORM_CLASS):
    ...
    def __init__(self, interface, confHandler, onCloseCallback):
        ...
        self.profileWin = ProfileDialog(
            self, self.iface, self.drawTool, self.projectHandler
        )
    ...
    def connectGuiElements(self):
        ...
        self.btnAdjustment.clicked.connect(
            lambda: self.onConfirm(runOptimization=False)
        )
        ...
        self.coordAx.editingFinished.connect(lambda: self.onCoordFieldChange("A"))
        self.coordAy.editingFinished.connect(lambda: self.onCoordFieldChange("A"))
        self.coordEx.editingFinished.connect(lambda: self.onCoordFieldChange("E"))
        self.coordEy.editingFinished.connect(lambda: self.onCoordFieldChange("E"))
        ...
        for name, inputField in self.parameterFields.items():
            ...
            if isinstance(inputField, QComboBox) and name == "Seilsys":
                inputField.currentIndexChanged.connect(self.getListenerComboBox(name))
            else:
                inputField.editingFinished.connect(self.getListenerLineEdit(name))
    ...
    def enableRasterHeightSource(self):
        ...
        self.labelDraw.setText(
            self.tr("Seillinie in Karte einzeichnen", "SeilaplanDialogUI")
        )
        self.labelCoords.setText(
            self.tr(
                "oder Koordinaten der Seillinie manuell angeben:",
                "SeilaplanDialogUI",
            )
        )
    ...
    def enableSurveyDataHeightSource(self):
        ...
        # Change label next to draw button
        self.labelDraw.setText(
            self.tr("Start- Endpunkt auf dem Gelaendeprofil definieren")
        )
        self.labelCoords.setText("")
        # Deactivate ui elements in group cableline
        self.toggleCableLineUI(False)
    ...
    def setFieldReadOnly(self, field, readonly):
        ...
        if readonly:
            ...
            field.setToolTip(
                self.tr(
                    "Koordinate kann nicht manuell veraendert werden, benutzen Sie die Schaltflaeche zeichnen"
                )
            )
        else:
            field.setStyleSheet("")
    ...
    def setupContentForFirstRun(self):
        ...
        # Set point types
        self.fieldTypeA.setCurrentIndex(self.projectHandler.getPointTypeAsIdx("A"))
        self.fieldTypeE.setCurrentIndex(self.projectHandler.getPointTypeAsIdx("E"))
    ...
    def onSaveParameterSet(self):
        if not self.paramHandler.checkValidState():
            return
        self.paramSetWindow.setData(
            self.paramHandler.getParametersetNames(),
        ...
        if msgBox.clickedButton() == yesBtn:
            ...
            if not success:
                QMessageBox.critical(
                    ...
                    self.tr("Parameterset loeschen"),
                    self.tr(
                        "Ein Fehler ist aufgetreten. Parameterset kann nicht geloescht werden."
                    ),
                    QMessageBox.StandardButton.Ok,
                )
            else:
    ...
    def setRaster(self, selectedRasters: list = None):
        ...
        rasterValid = (
            self.projectHandler.heightSource and self.projectHandler.heightSource.valid
        )
    ...
    def checkEqualSpatialRef(self):
        ...
        # Height source is in a geographic crs
            ...
            if hsType in ["dhm", "dhm_list"]:
                msg = self.tr(
                    "KBS-Fehler - Raster kann nicht verarbeitet werden"
    ...
    def onLoadSurveyData(self):
        ...
        if self.surveyImportDialog.doImport:
            ...
            self.projectHandler.setHeightSource(
                None,
                "survey",
    ...
    def loadSurveyData(self):
        ...
        if heightSource and heightSource.valid:
            ...
            self.surveyLineLayer, self.surveyPointLayer = createProfileLayers(
                heightSource
            )
            # Zoom to layer
    ...
    def onCoordFieldChange(self, pointType):
        ...
        [x, y], coordState, hasChanged = self.projectHandler.setPoint(pointType, [x, y])
        if hasChanged:
    ...
    def onSaveProject(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            ...
            os.path.join(
                ...
                self.projectHandler.getProjectName() + ".json",
            ),
            fFilter,
        )
    ...
    def onConfirm(self, runOptimization):
        if runOptimization and not (
            self.paramHandler.checkBodenabstand()
            and self.paramHandler.checkBodenabstandDistance(
                self.projectHandler.A_type, self.projectHandler.E_type
            )
        ):
            # Optimization can't run because there's a problem with the parameters
            return
        if (
            self.confHandler.checkValidState()
