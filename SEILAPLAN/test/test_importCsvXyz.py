...
class TestCsvXyzReader(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.csv_with_comments_path = os.path.join(
            TESTDATA_DIR, "Gelaendeprofil_XYZ_with_comments.csv"
        )
    ...
    def test_read_out_data_valid_csv(self):
        ...
        self.assertTrue(np.array_equal(reader.surveyPoints["x"], np.array([1.0, 4.0])))
        self.assertTrue(np.array_equal(reader.surveyPoints["y"], np.array([2.0, 5.0])))
        self.assertTrue(np.array_equal(reader.surveyPoints["z"], np.array([3.0, 6.0])))
    ...
    def test_csv_with_semicolons(self):
        ...
        self.assertTrue(
            np.array_equal(
                reader.surveyPoints["z"],
                np.array([200, 212, 248, 461, 464, 470, 791, 915, 921, 944, 1011]),
            )
        )
