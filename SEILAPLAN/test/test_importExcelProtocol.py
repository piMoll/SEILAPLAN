...
class TestExcelProtocolReader(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.testfile_missing_first_distance = os.path.join(
            TESTDATA_DIR, "Protocol_missing_first_distance.xlsx"
        )
        cls.testfile_ok_abs_point_at_end = os.path.join(
            TESTDATA_DIR, "Protocol_ok_abs_point_at_end.xlsx"
        )
        cls.testfile_abs_point_outside_measures = os.path.join(
            TESTDATA_DIR, "Protocol_abs_point_outside_measures.xlsx"
        )
        cls.testfile_missing_x_in_abs_point = os.path.join(
            TESTDATA_DIR, "Protocol_missing_x_in_abs_point.xlsx"
        )
        cls.testfile_missing_z_in_abs_point = os.path.join(
            TESTDATA_DIR, "Protocol_missing_z_in_abs_point.xlsx"
        )
    ...
    def test_readOutData_minimal_valid_protocol(self, mock_readxl):
        ...
        self.assertTrue(
            np.array_equal(reader.surveyPoints["x"], np.array([2600000, 2600000]))
        )
        self.assertTrue(
            np.array_equal(reader.surveyPoints["y"], np.array([1200000, 1200100]))
        )
    ...
    def test_readOutData_incomplete_first_measurements(self, mock_readxl):
        ...
        self.assertFalse(result)
        self.assertIn(
            f"Fehlende oder fehlerhafte Werte fuer Distanz oder Neigung auf Zeile {ExcelProtocolReader.ROW_START}",
            reader.errorMsg,
        )
    ...
    def test_readOutData_azimut_full_100(self, mock_readxl):
        ...
        mock_sheet.address.side_effect = lambda address: mock_protocol_azi_100.get(
            address, ""
        )
        mock_readxl.return_value.ws.return_value = mock_sheet
        ...
        self.assertTrue(
            np.array_equal(
                reader.surveyPoints["x"],
                np.array([2600000, 2600010, 2600020, 2600040]),
            )
        )
        self.assertTrue(
            np.array_equal(
                reader.surveyPoints["y"],
                np.array([1200000, 1200000, 1200000, 1200000]),
            )
        )
        self.assertTrue(
    ...
    def test_readOutData_azimut_full_300(self, mock_readxl):
        ...
        mock_sheet.address.side_effect = lambda address: mock_protocol_azi_300.get(
            address, ""
        )
        mock_readxl.return_value.ws.return_value = mock_sheet
        ...
        self.assertTrue(
            np.array_equal(
                reader.surveyPoints["x"],
                np.array([2600000, 2599990, 2599980, 2599960]),
            )
        )
        self.assertTrue(
            np.array_equal(
    ...
    def test_readOutData_azimut_full_400(self, mock_readxl):
        ...
        mock_sheet.address.side_effect = lambda address: mock_protocol_azi_400.get(
            address, ""
        )
        mock_readxl.return_value.ws.return_value = mock_sheet
        ...
        self.assertTrue(
            np.array_equal(
                reader.surveyPoints["x"],
                np.array([2600000, 2600000, 2600000, 2600000]),
            )
        )
        self.assertTrue(
            np.array_equal(
                reader.surveyPoints["y"],
                np.array([1200000, 1200010, 1200020, 1200040]),
            )
        )
        self.assertTrue(
            np.array_equal(reader.surveyPoints["z"], np.array([300, 300, 300, 300]))
    ...
    def test_file_full_protocol(self):
        ...
        self.assertTrue(
            np.array_equal(
                np.round(reader.surveyPoints["x"], 3),
                np.array(
                    [
                        2700020.873,
                        2700004.233,
        ...
        self.assertTrue(
            np.array_equal(
                np.round(reader.surveyPoints["y"], 3),
                np.array(
                    [
                        1299988.094,
                        1299997.586,
        ...
        self.assertTrue(
            np.array_equal(
                np.round(reader.surveyPoints["z"], 3),
                np.array(
                    [
                        993.132,
                        998.879,
