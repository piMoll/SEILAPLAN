...
class BirdViewSymbol:
    ...
    def mirror(self, axis="y"):
        if axis == "y":
            ...
            return self.mplPath.transformed(
                transforms.Affine2D().scale(-1, 1)
            ).transformed(transforms.Affine2D().scale(1, -1))
...
class BirdViewSymbolLoader:
    ...
    def loadSymbolFromArray(self, path=r"config/birdView"):
        ...
        for symbolName, svgProp in SVG_PROPS.items():
            ...
            try:
                vertices = np.load(
                    os.path.join(_HOME_PATH, path, f"{symbolName}_vertices.npy")
                )
                codes = np.load(
                    os.path.join(_HOME_PATH, path, f"{symbolName}_codes.npy")
                )
            ...
            self.symbols[symbolName] = BirdViewSymbol(
                symbolName, mplPath, scale, color, centerPoint
            )
        return self.symbols
    ...
    def loadSymbolsFromSvgText(self):
        ...
        for symbolName, svgProp in SVG_PROPS.items():
            ...
            mplPath = mplPath.transformed(
                transforms.Affine2D().translate(-1 * SVG_CENTER[0], -1 * SVG_CENTER[1])
            )
    ...
    def previewSymbols(self, symbolName=None):
        if symbolName:
            ...
            plt.plot(
                0,
                0,
                marker=marker.mplPath,
                markersize=marker.scale * 200,
                color=marker.color,
            )
        else:
    ...
    def saveSymbolsAsPaths(self, savePath=r"config/birdView"):
        for symbolName, symbol in self.symbols.items():
            np.save(
                os.path.join(_HOME_PATH, savePath, f"{symbolName}_vertices"),
                symbol.mplPath.vertices,
            )
            np.save(
                os.path.join(_HOME_PATH, savePath, f"{symbolName}_codes"),
                symbol.mplPath.codes,
            )
