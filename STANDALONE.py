...
def calculateFinalCableLine(conf: ConfigHandler):
    ...
    cable, force, cable_possible = preciseCable(
        parameters, conf.project.poles, optiResults["optSTA"]
    )
...
if __name__ == "__main__":
    ...
    if createOutput and status != ResultQuality.LineNotComplete:
        ...
        updateWithCableCoordinates(cableline, project.points["A"], project.azimut)
        ...
        imgPath = extractMapBackground(
            outputLoc, xlim, ylim, project.points["A"], project.azimut
        )
