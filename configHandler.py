...
SETTINGS_PATH = os.path.join(HOMEPATH, 'config')
SETTINGS_FILE = os.path.join(HOMEPATH, 'config', 'commonPaths.txt')
SETS_PATH = os.path.join(SETTINGS_PATH, 'parametersets')
USER_HOME_DIRECTORY = os.path.join(os.path.expanduser('~'))
DEFAULT_SAVE_PATH = os.path.join(USER_HOME_DIRECTORY, 'Seilaplan')
...
class ProjectConfHandler(AbstractConfHandler):
    ...
    def setConfig(self, property_name, value):
    ...
    def setDhm(self, rasterLyr):
        self.dhm = {}
        if rasterLyr:
            path = rasterLyr.grid.dataProvider().dataSourceUri()
            spatialRef = rasterLyr.grid.crs().authid()
            ext = rasterLyr.grid.extent()
            self.dhm['name'] = rasterLyr.name
            self.dhm['path'] = path
            self.dhm['layer'] = rasterLyr.grid
            self.dhm['ncols'] = rasterLyr.grid.width()
            self.dhm['nrows'] = rasterLyr.grid.height()
            self.dhm['cellsize'] = float(rasterLyr.grid.rasterUnitsPerPixelX())
    ...
    def getConfigAsStr(self):
        ...
        for key, values in list(self.fixedPoles['asStr'].items()):
            fixPolesStr += '{0:0>2}: {1: >7}, {2: >7}, {3: >4}  /  '.format(key, *tuple(values))
        ...
        txt = [
            ...
            [self.header['noPoleSections'], noPoleSectionStr]
        ]
        formattedProjectInfo = []
        for title, txt in txt:
...
class ParameterConfHandler(AbstractConfHandler):
    ...
    def _getParameter(self, property_name):
        try:
    ...
    def getParameter(self, property_name):
        p = self._getParameter(property_name)
        if not p:
            return ''
        value = p['value']
        if value is None:
    ...
    def setParameter(self, property_name, value):
        ...
        if cval != self.params[property_name]['value']:
            ...
            if property_name in ['d_Anker_A', 'd_Anker_E', 'HM_Anfang', 'HM_Ende_max']:
                valid = self.checkAnchorDependency(property_name, cval)
                if not valid:
                    return False
            
            if self.currentSetName and cval != self.parameterSets[self.currentSetName][property_name]:
                self.currentSetName = ''

        self.params[property_name]['value'] = cval
        
        return self.getParameter(property_name)
    ...
    def checkRange(self, value, paramInfo):
        ...
        for rangeItem in [rMin, rMax]:
            ...
            if any(r.isalpha() for r in rangeItem):
                ...
                rangeParam = self._getParameter(rangeItem)
                if not rangeParam:
                    return False
    ...
    def getParametersAsStr(self):
        """ """
        txt = ['{5}{5}{0}{5}{1: <17}{2: <12}{3: <45}{4: <9} {5:-<84}{5}'.format(
        ...
        for property_name, sortNr in self.paramOrder:
            ...
            value = self.getParameter(property_name)
    ...
    def loadPredefinedParametersets(self):
        for f in os.listdir(SETS_PATH):
            txtfile = os.path.join(SETS_PATH, f)
    ...
    def saveParameterSet(self):
        self.paramSetWindow.setData(self.parameterSets.keys(),
                                    SETTINGS_PATH)
        ...
        # Save parameter set
        self.parameterSets[setname] = {}
        for property_name, value in self.params.items():
            self.parameterSets[setname][property_name] = value
    
        # Write parameter set out to file
        savePath = os.path.join(SETTINGS_PATH, f'{setname}.txt')
    ...
    def getTruncatedParameters(self):
        # Short-hand dictionary for use in algorithm
        self.p = {}
...
class ConfigHandler(object):
    ...
    def __init__(self, interface):
        ...
        self.project = ProjectConfHandler()
        self.params = ParameterConfHandler(self.iface)
        ...
        if os.path.exists(filename):
            with io.open(filename, encoding='utf-8') as f:
                ...
                for hLine in lines[:6]:
                    ...
                    data = hLine[17:]
                    self.project.setConfig(property_name, data)
    ...
    def saveToFile(self, filename):
        project = self.project.getConfigAsStr()
        params = self.params.getParametersAsStr()
    ...
    def loadUserSettings(self):
        ...
        # If there are no paths defined by user (for example in first run),
        # try to create standard folder
        if not self.commonPaths:
            try:
                os.mkdir(DEFAULT_SAVE_PATH)
                self.commonPaths.append(DEFAULT_SAVE_PATH)
            except OSError:
                pass
    ...
    def updateUserSettings(self):
        ...
        with io.open(SETTINGS_FILE, encoding='utf-8', mode='w+') as f:
            f.writelines(
