...
class AdjustmentPlot(FigureCanvas):
    ...
    def __init__(
        ...
        self.fig = Figure(
            figsize=(width, height), dpi=self.dpi, facecolor=self.COLOR_PLOT_BG
        )
        ...
        if withBirdView:
            axes = self.fig.subplots(
                nrows=2,
                ncols=1,
                gridspec_kw={"height_ratios": [profilePlotRatio, 1]},
                sharex=True,
            )
            self.axes = axes[0]
    ...
    def __setupAxes(self):
        ...
        self.axes.grid(which="minor", color=self.COLOR_GRID, lw=1, linestyle=":")
        self.axes.tick_params(which="minor", direction=direction)

    def initData(self, xdata, terrain, peakLocation_x, peakLocation_y, surveyPoints):
        self.xdata = xdata
    ...
    def updatePlot(self, poles, cable):
        ...
        if self.tPoints is not None:
            ...
            for pointX, pointY, idx, notes in zip(
                ...
                self.axes.text(
                    pointX,
                    pointY - 7 * self.labelBuffer * scale,
                    labelText,
                    ha=ha,
                    va=va,
                    fontsize=fontSize,
        ...
        if not self.asPdf:
            ...
            self.axes.scatter(
                ...
                color="#ffaa00",
                edgecolors="#496b48",
                label=self.tr("Guenstige Gelaendeform"),
                zorder=3,
            )
        # Cable lines
        self.axes.plot(
        ...
        self.axes.plot(
            ...
            color="#FF4D44",
            linewidth=1.5 * scale,
            label=self.tr("Lastwegkurve nach Zweifel"),
        )
        # Anchors
        if cable["anchorA"]:
            self.axes.plot(
        ...
        for i, d in enumerate(pole_d):
            self.axes.plot(
                [pole_d[i], pole_dtop[i]],
                [pole_z[i], pole_ztop[i]],
                color="#363432",
                linewidth=3.0 * scale,
            )
            if poleType[i] == "crane":
    ...
    def showMarkers(self, plotMarkers: List[PlotMarker]):
        ...
        self.removeMarkers()
        spacing = (
            3 * abs(self.axes.get_ylim()[0] - self.axes.get_ylim()[1]) / self.height()
        )
        ...
        for marker in plotMarkers:
            ...
            self.arrowMarker.append(
                self.axes.scatter(
                    ...
                    marker=symbol,
                    zorder=20,
                    c=self.COLOR_MARKER[marker.color],
                    s=100,
                )
            )
    ...
    def layoutDiagrammForPrint(self, title, poles, direction):
        self.axes.set_title(
            self.tr("Seilaplan Plot  -  {}").format(title),
            fontsize=10,
    ...
    def createBirdView(self, poles, azimut):
        ...
        self.axesBirdView.plot(
            [poles[0]["d"], poles[-1]["d"]], [0, 0], color="red", linewidth=2.5
        )
        ...
        markerSize = (
            figExtent.height / (2 * yLim / markerDiameter) * 72.0 / self.fig.dpi
        )
