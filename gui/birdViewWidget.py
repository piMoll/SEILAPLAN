...
class BirdViewRow(object):
    ...
    def __init__(
        ...
        self.poleIndex = (
            poleIdx  # Pole number in Poles.poles array: can start at 0 or 1
        )
        ...
        self.defaultAbspannIdx = (
            0
            if self.rowIndex == 1
            or (
                self.parent.direction == "uphill"
                and (self.rowType not in ["anchor", "pole_anchor"])
            )
            else 2
        )
    ...
    def addFieldCat(self, value):
        ...
        self.fieldCat.currentIndexChanged.connect(
            lambda newVal: self.onCatSelection(
                self.poleIndex, model.item(newVal).data()
            )
        )
    ...
    def addFieldPos(self):
        ...
        self.fieldPos.currentIndexChanged.connect(
            lambda newVal: self.parent.onRowChange(
                self.poleIndex,
                "position",
                (self.fieldPosModel.item(newVal).data() if newVal > -1 else None),
            )
        )
    ...
    def addFieldAbspann(self):
        ...
        self.fieldAbspann.currentIndexChanged.connect(
            lambda newVal: self.parent.onRowChange(
                self.poleIndex,
                "abspann",
                (self.fieldAbspannModel.item(newVal).data() if newVal > -1 else None),
            )
        )
    ...
    def setFieldAbspannValue(self, dataValue):
        ...
        # Manually trigger the event
        dataValue = (
            None if newIndex == -1 else self.fieldAbspannModel.item(newIndex).data()
        )
    ...
    def onCatSelection(self, poleIdx, newCategory):
        ...
        self.setFieldPosValue(
            None if currentIdx == -1 else self.fieldPosModel.item(currentIdx).data()
        )
        ...
        self.setFieldAbspannValue(
            None if currentIdx == -1 else self.fieldAbspannModel.item(currentIdx).data()
        )
...
def shortenTextToFitLabel(labelField):
    ...
    if (
        labelField.fontMetrics().boundingRect(labelField.text()).width()
        > labelField.size().width()
    ):
