...
class CustomPoleWidget(QObject):
    ...
    def addHeader(self):
        ...
        self.layout.addWidget(
            QLabel(self.tr("Nr.")), 0, 1, 1, 1, Qt.AlignmentFlag.AlignLeft
        )
        self.layout.addWidget(
            QLabel(self.tr("Stuetzenbezeichnung")),
            0,
            2,
            1,
            1,
    ...
    def updatePoleRowNumbers(self):
        labels = None
        if self.poles:
            [_, _, _, _, _, labels, _, _, _, _] = self.poles.getAsArray(True, True)
    ...
    def updateNeighbourDistRange(self, idx, rightLimit, leftLimit):
        if idx > 0:
            ...
            self.poleRows[idx - 1].updateUpperDistRange(leftLimit - self.pole_dist_step)
    ...
    def updateAnchorState(self, prevAnchorA, prevAnchorE):
        ...
        if prevAnchorE is not self.poles.hasAnchorE:
            ...
            if self.poles.hasAnchorE:
                ...
                self.deactivateRow(
                    idxE, float(self.poleArr[idxE - 1]["d"]), self.distRange[1]
                )
...
class PoleRow(object):
    ...
    def addLabelNr(self, nr):
        ...
        self.layout.addWidget(
            self.labelNr, self.index + 1, 1, 1, 1, Qt.AlignmentFlag.AlignLeft
        )
    ...
    def addFieldName(self, value):
        ...
        self.layout.addLayout(
            hLayout, self.index + 1, 2, 1, 1, Qt.AlignmentFlag.AlignLeft
        )
        # Connect events
        self.fieldName.inFocus.connect(lambda x: self.parent.zoomIn(self.index))
    ...
    def addFieldDist(self, value, distRange):
        ...
        self.layout.addLayout(
            hLayout, self.index + 1, 3, 1, 1, Qt.AlignmentFlag.AlignLeft
        )

        # Connect events
        self.fieldDist.inFocus.connect(lambda x: self.parent.zoomIn(self.index))
    ...
    def addFieldHeight(self, value):
        ...
        self.layout.addLayout(
            layout, self.index + 1, 4, 1, 1, Qt.AlignmentFlag.AlignLeft
        )

        # Connect events
        self.fieldHeight.inFocus.connect(lambda x: self.parent.zoomIn(self.index))
    ...
    def addFieldAngle(self, value):
        ...
        self.layout.addLayout(
            layout, self.index + 1, 5, 1, 1, Qt.AlignmentFlag.AlignLeft
        )

        # Connect events
        self.fieldAngle.inFocus.connect(lambda x: self.parent.zoomIn(self.index))
    ...
    def addBtnPlus(self, createButton):
        ...
        icon = QIcon()
        icon.addPixmap(
            QPixmap(getAbsoluteIconPath("icon_addrow.png")),
            QIcon.Mode.Normal,
            QIcon.State.Off,
        )
        ...
        self.layout.addWidget(
            self.addBtn, self.index + 1, 0, 1, 1, Qt.AlignmentFlag.AlignLeft
        )
    ...
    def addBtnDel(self, createButton):
        ...
        self.layout.addWidget(
            self.delBtn, self.index + 1, 7, 1, 1, Qt.AlignmentFlag.AlignLeft
        )
