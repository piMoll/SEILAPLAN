...
"""
Source: https://gist.github.com/ALenfant/5491853

Yen's algorithm for igraph, adapted from Wikipedia's pseudocode.
The arguments are:
    graph: your igraph graph object (warning: the edge's id will change by
            using this function, so make a copy with gcopy if you want to
            keep them intact)
    source: source vertex
    target: target vertex
    num_k: number of shortest paths you want
    weights: name of the edge attribute that contain each edge's weight
...
def yen_igraph(graph, source, target, num_k, weights):
    ...
    A = [graph.get_shortest_paths(source, to=target, weights=weights,
                                  output="vpath")[0]]
    ...
    for k in range(1, num_k):
        ...
        for i in range(len(A[k-1])-1):
            ...
            spurPath = graph.get_shortest_paths(spurNode, to=target,
                                            weights=weights, output="vpath")[0]
