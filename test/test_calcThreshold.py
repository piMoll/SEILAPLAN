...
class TestCalcThreshold(unittest.TestCase):
    ...
    def test_threshold_update(self):
        ...
        thdUpdater.update(
            ...
            poles,
            profile,
            (quality == ResultQuality.SuccessfulOptimization),
        )
        ...
        for topic in thdUpdater.topics:
            ...
            if topic.id == "bodenabstand":
                ...
                self.assertEqual(
                    ...
                    ["8.0 m", "7.4 m", "9.1 m", "6.0 m", "9.0 m"],
                )
                self.assertEqual(
                    [marker.x for marker in markers], [27, 74, 131, 222, 258]
                )
                self.assertAlmostEqualList(
                    [marker.z for marker in markers],
                    [1249.04, 1235.15, 1217.35, 1188.68, 1176.07],
                )
                self.assertEqual([marker.color for marker in markers], [1, 1, 1, 3, 1])
                self.assertEqual(
                    [marker.alignment for marker in markers],
            ...
            if topic.id == "seilzugkraft":
                ...
                self.assertEqual(
                    [marker.color for marker in markers], [1, 1, 1, 1, 1, 1, 1]
                )
                self.assertEqual(
                    [marker.alignment for marker in markers],
                    [
            ...
            if topic.id == "lastseilknickwinkel":
                ...
                self.assertEqual(
                    [marker.x for marker in markers],
                    [40.0, 97.0, 182.0, 250.0, 290.0],
                )
                self.assertAlmostEqualList(
                    [marker.z for marker in markers],
                    [1243.49, 1226.79, 1203.73, 1178.07, 1156.16],
                )
            ...
            if topic.id == "bhd":
                ...
                self.assertEqual(
                    [marker.label for marker in markers],
                    ["36 cm", "39 cm", "36 cm", "39 cm", "40 cm", "65 cm"],
                )
                self.assertEqual(
                    [marker.x for marker in markers],
                    [40.0, 97.0, 182.0, 250.0, 290.0, 305.0],
                )
                self.assertAlmostEqualList(
                    [marker.z for marker in markers],
                    [1243.49, 1226.79, 1203.73, 1178.07, 1156.16, 1148.56],
                )
                self.assertEqual(
                    [marker.color for marker in markers], [1, 1, 1, 1, 1, 1]
                )
                self.assertEqual(
                    [marker.alignment for marker in markers],
                    [
            ...
            if topic.id == "leerseildurchhang":
                ...
                self.assertEqual(
                    [marker.label for marker in markers],
                    ["0.1 m", "0.1 m", "0.3 m", "0.2 m", "0.1 m", "0.0 m"],
                )
                self.assertEqual(
                    [marker.x for marker in markers],
                    [20, 68, 139, 216, 270, 297],
                )
                self.assertAlmostEqualList(
                    [marker.z for marker in markers],
                    [1248.95, 1235.63, 1211.94, 1191.59, 1164.86, 1153.03],
                )
                self.assertEqual(
                    [marker.color for marker in markers], [1, 1, 1, 1, 1, 1]
                )
            ...
            if topic.id == "lastseildurchhang":
                ...
                self.assertEqual(
                    [marker.label for marker in markers],
                    ["3.3 m", "4.4 m", "6.3 m", "5.2 m", "3.8 m", "2.6 m"],
                )
                self.assertEqual(
                    [marker.x for marker in markers],
                    [20, 68, 139, 216, 270, 297],
    ...
    def test_threshold_update_with_empty_extrema(self):
        conf: ConfigHandler = ConfigHandler()
        result, params, poles, profile, quality = calculate_cable_line(
            conf, MINIMAL_PROJECT_FILE
        )
