...
TEST_PROJECT_A_A_UPHILL = project_file_loader(
    "unittest_dhm_anchor_anchor_4_poles_uphill.json"
)
...
class TestPolesAngriffswinkel(unittest.TestCase):
    ...
    def test_anchor_anchor(self):
        ...
        poles: Poles
        result, params, poles, profile, status = calculate_cable_line(
            conf, TEST_PROJECT_A_A
        )
...
class TestPolesBundstelleAndBHD(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.poles: Poles
        cls.result, cls.params, cls.poles, cls.profile, cls.status = (
            calculate_cable_line(cls.conf, TEST_PROJECT_A_A)
        )
