...
def optimization(IS, gp, StuetzenPos, progress):
    ...
    for sk in range(min_SK, zul_SK+1):
        try:
            ...
            dist, pred = sps.csgraph.dijkstra(Weight, directed=True,
                                              indices=0,
    ...
    return Loesung_HM, stueIdx, Value, OptSTA, optiLen#, waitDiag, stepBar
    # vorher OptSTA[:-2]

# dist = dist[LengthInLP] ist bei u = 156/157 falsch, dort dürfte die maximale dist nicht 36745 sondern kleiner sein
# Grund: etwas andere Min/Max Werte beim STA Wert (MinSTAMatrix) --> aufgrund Interpolation