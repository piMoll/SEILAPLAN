...
class ThresholdUpdater:
    ...
    def update(
        self,
        resultData,
        params: ParameterConfHandler,
        poles,
        profile,
        resultFromOptimization,
    ):
        self.poles = poles
        ...
        for row, topic in enumerate(self.getThresholdTopics()):
            self.layout.colorBackground(
                row, topic.getColumnIndexToColor(), topic.getMaxColor()
            )

        # Update warn icon of the tap title
        self.layout.updateTabIcon(
            any([item.exceedsThreshold for item in self.getThresholdTopics()])
        )
    ...
    def setThreshold(self, params: ParameterConfHandler):
        for topic in self.topics:
            ...
            if topic.id == "lastseilknickwinkel":
                topic.threshold = [
                    params.getParameter("LastKnickSt"),
                    params.getParameter("LastKnickEnd"),
                ]
            if topic.id == "leerseilknickwinkel":
                topic.threshold = [
                    params.getParameter("LeerKnickMit"),
                    params.getParameter("LeerKnickOhne"),
                ]
    ...
    def initThresholdItems(self, params: ParameterConfHandler):
        ...
        lastseilknickwinkel = PlotTopic(
            ...
            description={
                ...
                "message": self.tr(
                    "Groessere Knickwinkel reduzieren die Bruchlast des Tragseils und fuehren zu hoeheren Sattelkraeften. Hinweis: Bei den beiden Endmasten liegt der Grenzwert hoeher _threshold_2_"
                ),
            },
        )
        leerseilknickwinkel = PlotTopic(
            ...
            description={
                ...
                "message": self.tr(
                    "Bei Knickwinkeln unter _threshold_1_ besteht die Gefahr, dass das Tragseil beim Sattel abhebt (rot). Bei Knickwinkeln zwischen _threshold_1_ und _threshold_2_ muss das Tragseil mittels Niederhaltelasche gesichert werden (orange)."
                ),
            },
        )
    ...
    def checkThreshold(self, topic, resultData):
        ...
        if topic.id == "bodenabstand":
            ...
            for idx in range(0, len(dtop) - 1):
                ...
                d_diff = np.absolute(
                    resultData["cableline"]["groundclear_di"] - dtop[idx]
                )
                ...
                localMinIdx = np.ravel(
                    np.argwhere(
                        resultData["cableline"]["groundclear_rel"][
                            fieldStartIdx:fieldEndIdx
                        ]
                        == localMinima
                    )
                )
                xLoc = resultData["cableline"]["groundclear_di"][
                    fieldStartIdx:fieldEndIdx
                ][localMinIdx]
                zLoc = resultData["cableline"]["groundclear"][
                    fieldStartIdx:fieldEndIdx
                ][localMinIdx]
        ...
        # Max force on cable
            ...
            maxCableForce = resultData["force"]["MaxSeilzugkraft"][
                0
            ]  # Max force on cable
            ...
            forceAtHighestPoint = resultData["force"]["MaxSeilzugkraft_L"][
                0
            ]  # Cable force at highest point
            plotLabel = (
                self.tr("am hoechsten Punkt:")
                + "\n"
    ...
    def getZCoordinateFromTerrain(self, xLocation):
        return self.profile.zi_disp[
            np.argwhere(self.profile.di_disp == xLocation)[0][0]
        ]
