...
class ParameterConfHandler(AbstractConfHandler):
    ...
    SEILSYS_TYPES = {0: "Zweiseil-System", 1: "Mehrseil-System"}
    ANCHOR_LEN = 20
    ...
    def initParameters(self):
        ...
        self.derievedParams["d_Anker_A"] = {"value": self.ANCHOR_LEN}
        self.derievedParams["d_Anker_E"] = {"value": self.ANCHOR_LEN}
    ...
    def readParamsFromTxt(self, path):
        ...
        if (
            ...
            msg = self.tr(
                "Fehler in Parameterset '{}' gefunden. "
                "Set kann nicht geladen werden."
            ).format(path)
            self.onError(msg)
            return False
    ...
    def setParameter(self, property_name, value):
        ...
        if cval is not None and cval != self.params[property_name]["value"]:
            ...
            if (
                self.currentSetName
                and cval != self.parameterSets[self.currentSetName][property_name]
            ):
                self.currentSetName = ""
    ...
    def batchSetParameter(self, property_name, value):
        ...
        """
        property_name, value = self.checkForDeprecatedParams(property_name, value)
        p = self._getParameterInfo(property_name)
        if not p:
            self.onError(
                self.tr(
    ...
    def checkRange(self, value, paramInfo):
        ...
        for rangeItem in [rMin, rMax]:
            ...
            if any(r.isalpha() for r in rangeItem):
                ...
                if not rangeParam:
                    return False
                rangeVal = self.castToNumber(rangeParam["dtype"], rangeParam["value"])
            else:
                # Range is a value
        ...
        if value is None:
            errorMsg = self.tr("Bitte geben Sie im Feld einen Wert ein").format(
                self.tr(paramInfo["label"], "@default"),
                rangeSet[0],
                rangeSet[1],
                paramInfo["unit"],
            )
    ...
    def checkBodenabstand(self):
        if self.params["Bodenabst_min"]["value"] > self.params["HM_min"]["value"]:
            self.onError(
                self.tr(
                    "Der Parameter Minimaler Abstand Tragseil Boden darf nicht groesser als der Parameter Minimale Stuetzenhoehe sein."
                ),
    ...
    def checkBodenabstandDistance(self, poleType_A, poleType_E):
        if (
            self.params["Bodenabst_A"]["value"] == 0 and poleType_A == "pole_anchor"
        ) or (self.params["Bodenabst_E"]["value"] == 0 and poleType_E == "pole_anchor"):
            self.onError(
                self.tr(
    ...
    def prepareForCalculation(self, profileDirection=None):
        ...
        self.derievedParams["min_SK"] = {"value": int(round(SK * 0.85))}
    ...
    def updateAnchorLen(self, buffer, poletype_A, poletype_E):
        ...
        self.derievedParams["d_Anker_A"] = {"value": anchor_A}
        self.derievedParams["d_Anker_E"] = {"value": anchor_E}
