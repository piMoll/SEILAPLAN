...
class ProjectConfHandler(AbstractConfHandler):
    ...
    POINT_TYPE = {0: "pole_anchor", 1: "pole", 2: "crane"}
    ...
    def setConfigFromFileOld(self, property_name, value):
        ...
        if property_name == "Projektname":
            ...
            for stue in polesStr:
                ...
                [poled, polez, poleh] = [string.strip() for string in values.split(",")]
                poleArray.append(
                    {
                        "d": int(poled),
                        "z": float(polez),
    ...
    def setPoint(self, pointType, coords):
        ...
        if not (x == self.points[pointType][0] and y == self.points[pointType][1]):
            hasChanged = True
    ...
    def checkCoordinatePoint(self, coords):
        ...
        if (
            self.heightSource
            and self.heightSource.extent
            and x is not None
            and y is not None
        ):
            [extLx, extHy, extHx, extLy] = [
                round(e, precision) for e in self.heightSource.extent
            ]

            # Round coordinates to avoid float imprecision
            if min(extLx, extHx) <= x <= max(extLx, extHx) and min(
                extLy, extHy
            ) <= y <= max(extLy, extHy):
                state = "green"
            else:
    ...
    def setAzimut(self):
        ...
        if self.profilePointsAreValid():
            azimut = atan2(
                self.points["E"][0] - self.points["A"][0],
    ...
    def setProfileLen(self):
        ...
        if self.profilePointsAreValid():
            ...
            if crs and crs.isGeographic():
                ...
                length = distance.measureLine(
                    QgsPointXY(*tuple(self.points["A"])),
                    QgsPointXY(*tuple(self.points["E"])),
                )
            else:
                length = (
                    (self.points["E"][0] - self.points["A"][0]) ** 2
                    + (self.points["E"][1] - self.points["A"][1]) ** 2
                ) ** 0.5
    ...
    def setFixedPoles(self, value):
        ...
        for pole in value:
            ...
            if not h:
                h = -1
            self.fixedPoles["poles"].append({"d": d, "z": z, "h": h, "name": name})
    ...
    def getSettings(self):
        ...
        return {
            ...
            "profile": {
                ...
                "end": {"coordinates": self.points["E"], "type": self.E_type},
    ...
    def checkValidState(self):
        ...
        if not self.projectName:
            ...
            if self.heightSourceType == "survey":
                msg = self.tr(
                    "Bitte zeichnen Sie Start- und Endpunkt der Seillinie in die Karte ein (Schaltflaeche zeichnen)"
                )
            else:
                msg = self.tr(
                    "Bitte zeichnen Sie die Seillinie in die Karte (Schaltflaeche zeichnen) oder definieren sie Start- und Endkoordinaten manuell"
                )
        if msg:
    ...
    def updatePoles(self):
        ...
        #  (if the saved file included fixed poles, they would already be part of self.poles)
            ...
            self.poles.updateAllPoles(
                PolesOrigin.OnlyStartEnd, self.fixedPoles["poles"]
            )
