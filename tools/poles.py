...
class Poles(object):
    ...
    def __init__(self, project):
        ...
        if self.E_type == "pole":
            ...
            self.add(
                idx + 2,
                self.profileLength + self.anchorE,
                0,
                poleType="anchor",
                refresh=False,
            )
    ...
    def update(self, idx, property_name, newVal):
        ...
        if property_name in ["d", "h", "z", "angle"]:
            x, y, z, dtop, ztop = self.derivePoleProperties(
                self.poles[idx]["d"],
                self.poles[idx]["h"],
        ...
        if mode == PolesOrigin.Optimization:
            ...
            for p in poles:
                self.add(
                    ...
                    p["abspann"] if "abspann" in p else None,
                    False,
                )
    ...
    def setDirection(self):
        ...
        if self.firstPole and self.lastPole:
            self.direction = (
                "downhill"
                if self.firstPole["ztop"] > self.lastPole["ztop"]
                else "uphill"
            )
    ...
    def calculateAnchorLength(self):
        ...
        anchor_len = (poleA_dtop_diff**2 + poleA_ztop_diff**2) ** 0.5 + (
            poleE_dtop_diff**2 + poleE_ztop_diff**2
        ) ** 0.5

        self.anchor = {"field": anchor_field, "len": anchor_len}
    ...
    def getCableFieldDimension(self):
        [_, _, _, dtop, ztop, _, _, _, _, _] = self.getAsArray(
            withAnchor=True, withDeactivated=True
        )
    ...
    def getAnchorCable(self):
        anchorFieldA = None
        anchorFieldE = None
        [_, _, _, pole_dtop, pole_ztop, _, _, _, _, _] = self.getAsArray(True, False)

        if self.hasAnchorA:
            anchorFieldA = {"d": pole_dtop[:2], "z": pole_ztop[:2]}
        if self.hasAnchorE:
            anchorFieldE = {"d": pole_dtop[-2:], "z": pole_ztop[-2:]}
        return {"A": anchorFieldA, "E": anchorFieldE}

    def updateAnchorState(self):
        self.hasAnchorA = (
            self.poles[0]["poleType"] == "anchor" and self.poles[0]["active"]
        )
        self.hasAnchorE = (
            self.poles[-1]["poleType"] == "anchor" and self.poles[-1]["active"]
        )
    ...
    def calculateAdvancedProperties(self, forces, bundstelle):
        ...
        for j, pole in enumerate(self.poles):
            ...
            # Start or end pole with h > 0 --> not passable
            elif pole["poleType"] == "pole" and pole in [
                self.firstPole,
                self.lastPole,
            ]:
            ...
            # pole in between --> passable
                ...
                # Check if array only contains nan values
                if not np.all(
                    np.isnan(forces["Sattelkraft_beiStuetze"][0][idx * 2 : idx * 2 + 2])
                ):
                    maxForceNB = np.nanmax(
                        forces["Sattelkraft_beiStuetze"][0][idx * 2 : idx * 2 + 2]
                    )
