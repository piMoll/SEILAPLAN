...
class SurveyData(AbstractHeightSource):
    ...
    def openFile(self, sourceType):
        ...
        if not os.path.exists(self.path):
            self.valid = False
            self.errorMsg = self.tr("Datei '_path_' ist nicht vorhanden.")
        ...
        if success:
            ...
            if not self.errorMsg:
                self.errorMsg = self.tr(
                    "Datei konnte nicht eingelesen werden. Unerwarteter Fehler aufgetreten."
                )
    ...
    def projectOnLine(self):
        ...
        projOntoPlane = (
            pointsFromPointInPlane - projOntoNormalVector[:, None] * self.normalVector
        )
        ...
        self.plotNotes = (
            self.plotNotes if len(self.plotNotes) == len(self.x) else [""] * len(self.x)
        )
        ...
        points = {"A": self.getFirstPoint(), "E": self.getLastPoint()}
